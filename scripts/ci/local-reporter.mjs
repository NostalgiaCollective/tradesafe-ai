...
export default class LocalReporter {
  ...
  onTestEnd(test, result) {
    const lines = [...new Set(result.errors.flatMap(e => [...(e.stack || '').matchAll(/(?:(?:workflows|restored|follow-up|trades|concerns|crew|sites|daily-brief)\.spec|follow-up|concerns|crew|sites|daily-brief|site-signin)\.mjs:(\d+)/g)].map(m => Number(m[1]))))]
  ...
  onEnd(result) {
    ...
    const status = result.status === 'passed' && this.rows.length === (recovery ? 1 : 12) && !skipped ? 'passed' : 'failed'
