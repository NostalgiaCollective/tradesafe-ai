...
}catch(e){state.status='FAIL';state.failure={name:e.name,message:e.message.slice(0,1800),lines:[...(e.stack||'').matchAll(/report-usability\.mjs:(\d+)/g)].map(m=>+m[1])};await page.screenshot({path:out+'/failure.png',fullPage:true}).catch(()=>{});process.exitCode=1}
