...
try{
 ...
 await p.goto(origin+'/report/'+result.reportId+'?step=3',{waitUntil:'networkidle'})
 ...
 await p.getByRole('button',{name:'Review observations',exact:true}).click();await p.getByRole('checkbox').check();await p.getByRole('button',{name:'Finalize observations',exact:true}).click();await expect(p.getByRole('heading',{name:'Retained PDF export',exact:true})).toBeVisible();pass('Only dedicated synthetic report finalized')
 }
