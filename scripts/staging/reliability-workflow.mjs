...
try{
 ...
 if(!has('finalization is immutable and PDF is retained and authorized')){
  ...
  if(await fields.count()){for(let i=0;i<await fields.count();i++){await fields.nth(i).selectOption(i===0?'attention':'meets');if(i===0)await p.getByLabel('Explanation (required)',{exact:true}).fill('SYNTHETIC concern for action retry');await expect(p.locator('.save-state')).toHaveText('Saved')}await p.getByRole('button',{name:'Review observations',exact:true}).click();await p.getByRole('checkbox').check();await p.getByRole('button',{name:'Finalize observations',exact:true}).evaluate(b=>{b.click();b.click()});await expect(p.getByRole('heading',{name:'Retained PDF export',exact:true})).toBeVisible()}
  await p.reload({waitUntil:'networkidle'});if(await p.getByRole('button',{name:'Generate retained PDF',exact:true}).count()){await p.getByRole('button',{name:'Generate retained PDF',exact:true}).evaluate(b=>{b.click();b.click()})}
  await expect(p.getByRole('button',{name:'Download retained PDF',exact:true})).toBeEnabled();const download=p.waitForEvent('download');await p.getByRole('button',{name:'Download retained PDF',exact:true}).click();await download;await expect(p.getByRole('status').filter({hasText:'PDF sent to your browser'})).toBeVisible()
 ...
 if(!has('action lost-response retry is idempotent and confirmation is nearby')){
  await p.goto(origin+'/actions?company='+state.companyId,{waitUntil:'networkidle'});let lost=false,ids=[]
  ...
  await p.getByLabel('Immediate controls or action taken',{exact:true}).fill('SYNTHETIC controls retained on retry');await p.getByRole('button',{name:'Save action update',exact:true}).evaluate(b=>{b.click();b.click()});await expect(p.getByRole('alert').filter({hasText:'No completion was confirmed'})).toBeVisible();await p.getByRole('button',{name:'Save action update',exact:true}).click();await expect(p.getByRole('status').filter({hasText:'Action update saved.'}).last()).toBeVisible();assert.equal(ids[0],ids[1]);await c.unroute(origin+'/api/workspace');await p.reload({waitUntil:'networkidle'});await expect(p.getByLabel('Immediate controls or action taken',{exact:true})).toHaveValue('SYNTHETIC controls retained on retry');pass('action lost-response retry is idempotent and confirmation is nearby')
 }
 if(!has('amendment repeated taps preserve original retained PDF')){
  await p.goto(report,{waitUntil:'networkidle'});await p.getByLabel('Reason for amendment',{exact:true}).fill('SYNTHETIC reliability amendment');const response=p.waitForResponse(r=>r.url()===origin+'/api/workspace'&&r.request().postDataJSON()?.command==='amend');await p.getByRole('button',{name:'Create amendment',exact:true}).evaluate(b=>{b.click();b.click()});const r=await response;assert.equal(r.status(),200);state.amendmentId=(await r.json()).id;save();await p.waitForURL('**/report/'+state.amendmentId);const bytes=await (await c.request.get(origin+'/api/reports/'+state.reportId+'/exports/'+state.exportId)).body();assert.equal(createHash('sha256').update(bytes).digest('hex'),state.pdfHash);pass('amendment repeated taps preserve original retained PDF')
 }
