...
const {env}=requireStaging(),hosted=process.argv.includes('--hosted'),origin=hosted?HOSTED_ORIGIN:'https://localhost:3000',run=(hosted?'hosted':'local')+'-'+randomUUID().slice(0,8),file='.staging/crew-'+run+'.json',out='test-results/crew-'+run
...
const {chromium}=await import('@playwright/test'),{crewWorkflow,prepareCrew}=await import('../../tests/fixtures/crew.mjs'),browser=await chromium.launch(),gate=JSON.parse(readFileSync('.staging/hosted-access.json')),options=hosted?{httpCredentials:{username:gate.username,password:gate.password}}:{},actors={}
try{
 ...
 await crewWorkflow({browser,origin,actors,options,record:(name,data={})=>{result.checks.push(name);Object.assign(result,data);save();console.log(name)},capture:async(page,name)=>{for(const [width,label] of [[390,'phone'],[1366,'desktop']]){await page.setViewportSize({width,height:width===390?844:900});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);await page.screenshot({path:out+'/'+name+'-'+label+'.png',fullPage:true})}await page.setViewportSize({width:390,height:844})}})
 if(hosted){const phone=await prepareCrew(actors,'SYNTHETIC phone crew');result.phoneFixture=phone;writeFileSync('.staging/crew-phone-private.json',JSON.stringify({company:phone.company,siteId:phone.siteId,briefId:phone.briefId,actionId:phone.actionId,origin,worker:{email:actors.WORKER.email,password:actors.WORKER.password},supervisor:{email:actors.SUPERVISOR.email,password:actors.SUPERVISOR.password},workerUrl:origin+'/my-work?company='+phone.company,supervisorUrl:origin+'/my-work?company='+phone.company+'&view=crew'},null,2))}
 result.status='PASS'
}catch(e){result.status='FAIL';result.failure={name:e.name,assertion:e.matcherResult?.name||null,lines:[...(e.stack||'').matchAll(/crew\.mjs:(\d+)/g)].map(m=>+m[1])};process.exitCode=1}
