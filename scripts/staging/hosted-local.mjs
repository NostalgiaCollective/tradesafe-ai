...
finally{server?.kill();writeFileSync(output+'/preflight.json',JSON.stringify(result,null,2));const p='.staging/physical-phone-checkpoint.json',s=JSON.parse(readFileSync(p));s.localPreflight=result;s.nextStep=result.status==='PASS'?'Complete proposal and request hosting/Vercel-suppression approval; Render login/no-charge eligibility pending':'Fix failed local preflight before publication approval';writeFileSync(p,JSON.stringify(s,null,2))}
