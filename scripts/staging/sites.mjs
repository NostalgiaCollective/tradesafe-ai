...
const {env}=requireStaging(),hosted=process.argv.includes('--hosted'),origin=hosted?HOSTED_ORIGIN:'https://localhost:3000',run=(hosted?'hosted':'local')+'-'+randomUUID().slice(0,8),file='.staging/sites-'+run+'.json',out='test-results/sites-'+run
...
const {chromium}=await import('@playwright/test'),{siteWorkflow}=await import('../../tests/fixtures/sites.mjs'),browser=await chromium.launch(),gate=JSON.parse(readFileSync('.staging/hosted-access.json')),options=hosted?{httpCredentials:{username:gate.username,password:gate.password}}:{},actors={}
try{
