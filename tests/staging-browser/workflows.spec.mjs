...
test('Owner invitation UI, intended acceptance, company switching and removal with an existing browser session', async ({ page, browser }, info) => {
  ...
  try {
    ...
    await expect(invited.getByRole('button', { name: 'Accept invitation', exact: true })).toBeEnabled()
    expect(new URL(invited.url()).hash).toBe('')
    await invited.getByRole('button', { name: 'Accept invitation', exact: true }).click()
    await expect(invited.locator('main').getByRole('alert')).toContainText('different verified email')
