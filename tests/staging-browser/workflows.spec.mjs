...
test('Fresh company onboarding through the interface', async ({ page }, info) => {
  ...
  await expect(page.locator('.work-footnote').first()).toHaveText('SYNTHETIC first company')
})
...
test('Durable drafts: lost responses, queued edits, network retry, refresh and concurrent tabs', async ({ page, context }) => {
  ...
  try {
    ...
    await page.getByRole('button', { name: 'Create saved draft', exact: true }).click()
    await expect(page.locator('main').getByRole('alert')).toContainText('No completion was confirmed')
    ...
    await expect(page.locator('main').getByRole('alert')).toContainText('Keep this page open')
    await expect(page.locator('.work-footnote').filter({hasText:'offline persistence'})).toContainText('offline persistence is not provided')
    ...
    await expect(other.locator('main').getByRole('alert')).toContainText('newer version')
...
test('Truthful review, immutable amendment, payment endpoints, action verification and membership removal', async ({ page, browser }, info) => {
  ...
  try {
    ...
    await page.getByText('Optional contact and trade details',{exact:true}).click()
    ...
    await page.getByText('Company details saved with this report',{exact:true}).click();await expect(page.getByText('contact phone: 555-0105')).toBeVisible()
    ...
    await page.getByRole('button', { name: /2.*Observations/ }).click()
    ...
    await page.getByRole('button', { name: /4.*Finalize/ }).click()
    await expect(page.getByRole('button', { name: 'Finalize report', exact: true })).toBeDisabled()
    await page.getByRole('button', { name: /2.*Observations/ }).click()
    for (const group of await page.locator('.observation-section h3 button').all()) if(await group.getAttribute('aria-expanded')==='false') await group.click()
    ...
    await page.getByLabel('Immediate controls or action taken (optional)', { exact: true }).fill('SYNTHETIC area kept clear')
    await saved(page)
    ...
    await page.getByRole('button', { name: /4.*Finalize/ }).click()
    await expect(page.getByRole('heading', { name: '1 concerns will remain recorded' })).toBeVisible()
    ...
    await page.getByLabel('Reason for correction', { exact: true }).fill('SYNTHETIC correction')
    ...
    await supervisorPage.getByLabel('Whose actions?', { exact: true }).selectOption('0')
    await supervisorPage.getByRole('button',{name:'Show actions',exact:true}).click()
    await supervisorPage.getByText('Update progress', {exact:true}).click()
    await supervisorPage.getByText('Assignment, due date and immediate controls',{exact:true}).click()
    await supervisorPage.getByLabel('Responsible member', { exact: true }).selectOption(f.clients.SUPERVISOR.user.id)
    await supervisorPage.getByLabel('Progress or resolution notes', { exact: true }).fill('SYNTHETIC repair checked')
    await supervisorPage.getByLabel('Status after saving', { exact: true }).selectOption('closed')
    await supervisorPage.getByRole('button', { name: 'Verify and close', exact: true }).click()
    await expect(supervisorPage.getByRole('status').filter({hasText:'Action update saved.'}).first()).toBeVisible()
    // The selected action stays visible after closure, with its list filters preserved.
    await supervisorPage.getByLabel('Status after saving', { exact: true }).selectOption('open')
    const reopenCommitted = deferred(), releaseReopen = deferred()
    await supervisorPage.route('**/api/workspace', async route => {
      if (route.request().postDataJSON().command === 'update_action') {
        const response = await route.fetch()
        expect(response.status()).toBe(200)
        reopenCommitted.resolve()
    ...
    try {
      await expect(supervisorPage.getByRole('status').filter({hasText:'Action update saved.'})).toHaveCount(0)
      await expect(supervisorPage.getByRole('status').filter({hasText:'Saving action update'})).toBeVisible()
      await expect(supervisorPage.getByRole('button', { name: /^Saving/  })).toBeDisabled()
    } finally { releaseReopen.resolve() }
    await expect(supervisorPage.getByRole('status').filter({hasText:'Action update saved.'}).first()).toBeVisible()
    await supervisorPage.unrouteAll({ behavior: 'wait' })
...
test('Session loss preserves in-memory edits; a different account does not inherit the editor', async ({ page, context }) => {
  ...
  try {
    ...
    await expect(page.locator('main').getByRole('alert')).toContainText(/sign in/i)
...
test('Owner invitation UI, intended acceptance, company switching and removal with an existing browser session', async ({ page, browser }, info) => {
  ...
  try {
    ...
    await expect(ownRow.getByRole('button', { name: 'Remove access', exact: true })).toBeDisabled()
    await expect(page.getByText('Add another owner before changing or removing the last owner.', { exact: true })).toBeVisible()
    await page.getByLabel('Their sign-in email', { exact: true }).fill(env.STAGING_OUTSIDER_EMAIL)
    await page.locator('#invite-role').selectOption('worker')
    ...
    await expect(invited.getByRole('status').filter({ hasText: 'another verified email' })).toBeVisible()
    expect(new URL(invited.url()).hash).toBe('')
    await expect(invited.getByRole('button', { name: 'Accept invitation', exact: true })).toHaveCount(0)
    await login(invited, 'OUTSIDER')
    await invited.goto(privateLink)
    await invited.getByRole('button', { name: 'Accept invitation', exact: true }).click()
    await invited.getByRole('link', { name: 'Open company reports', exact: true }).click()
    ...
    await invited.locator('.company-picker>summary').click()
