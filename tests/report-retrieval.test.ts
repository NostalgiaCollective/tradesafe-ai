...
import {listReturn,reportFilters,reportListUrl,searchPattern} from '../lib/domain/report-list.ts'
...
test('report search is bounded and return navigation preserves only same-company list filters',()=>{
 ...
 for(const bad of ['https://evil.test/dashboard?company='+company,'//evil.test','/actions?company=other','/dashboard?company='+company+'#x'])assert.equal(listReturn(bad,company),'/dashboard?company='+company)
