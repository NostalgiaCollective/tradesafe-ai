...
export default async function BriefPage({params,searchParams}){
 ...
 return <WorkspaceShell {...w}>{p.from&&safeRedirect(p.from).startsWith('/actions?')&&new URL(p.from,'https://internal.invalid').searchParams.get('company')===w.company.id&&<p><a href={safeRedirect(p.from)}>Back to actions</a></p>}<RecordSite supabase={supabase} companyId={w.company.id} actor={user.id} id={id} kind="brief" editable={editable}/><p><a href={'/briefs?company='+w.company.id}>Back to daily briefs</a></p><p className="work-notice">Draft safety content — staging only, pending qualified review. Recording this brief does not certify compliance or authorize work. App roles do not establish constructor, supervisor or competent-person status under law.</p>{data.brief.lifecycle==='draft'&&!p.version?<BriefEditor key={id+data.brief.revision} initial={data.brief} contentCatalog={data.contentCatalog} actor={user.id} members={data.members} editable={editable} initialStep={Number(p.step)||1} previousVersion={data.versions[0]?.version}/>:<BriefRecord {...data} actor={user.id} role={w.membership.role} editable={editable}/>}</WorkspaceShell>
}
