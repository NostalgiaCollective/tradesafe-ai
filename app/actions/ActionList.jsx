...
function Action({initial,members,events,role,actor,companyId,onSaved}) {
 ...
 <form onSubmit={save}><fieldset disabled={!editable||busy} className="work-fieldset">
 ...
 <label htmlFor={'resolution-'+action.id}>Resolution notes (required for verification)</label><textarea id={'resolution-'+action.id} maxLength={4000} value={edit.resolution} onChange={e=>setEdit({...edit,resolution:e.target.value})}/>
 ...
 <button disabled={busy||!editable||(action.state==='closed'&&edit.state!=='open')}>{busy?'Saving...':action.state==='closed'?'Reopen action':'Save action update'}</button></fieldset>{error&&<p role="alert">{error}</p>}</form>
