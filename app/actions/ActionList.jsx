...
function Action({initial,members,events,role,actor,companyId,onSaved,onSaving,hidden,returnTo}){
 ...
 async function save(e){e.preventDefault();if(lock.current||!editable)return;lock.current=true;setBusy(true);setError('');setErrorCode('');setUpdated(false);onSaving();try{
  ...
  const value={...await command('update_action',pending.current,actor),report:action.report};pending.current=null;setUncertain(false);setAction(value);setEdit(value);setHistory([]);setUpdated(true);onSaved(value)
 ...
 const name=id=>members.find(m=>m.user_id===id)?.display_name||'Former or unavailable member',blocked=['conflict','denied','immutable','not_found','account_changed'].includes(errorCode)
 return <article hidden={hidden} className="work-panel action-card" id={'action-'+action.id}>
 <h2><Link href={'/report/'+action.report_id+'?'+new URLSearchParams({from:returnTo})}>{action.report?.document?.job?.address||'Original report'}{action.report?.amendment_of?' (amendment)':''}</Link></h2>
