...
export default function ReportTools({report,canAmend,returnTo}){
 ...
 function amend(e){e.preventDefault();void operation.run('Creating an amendment. The original report will remain unchanged.',async()=>{
  ...
  const r=await command('amend',attempt.current);setSaved(r.id);router.push('/report/'+r.id+(returnTo?'?'+new URLSearchParams({from:returnTo}):''));return 'Amendment created. Opening the new draft.'
 })}
