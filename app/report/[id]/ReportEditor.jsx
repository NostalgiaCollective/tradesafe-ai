...
export default function ReportEditor({report,actor,editable,initialStep=2}) {
 ...
 {[['address','Job address','text'],['client','Client or job reference (optional)','text'],['date','Work date','date']].map(([key,label,type])=><div key={key}><label htmlFor={'job-'+key}>{label}</label><input id={'job-'+key} type={type} maxLength={1000} value={doc.job[key]} onChange={e=>draft.change({...doc,job:{...doc.job,[key]:e.target.value}})}/></div>)}
 ...
 <label htmlFor={'note-'+item.id}>{['attention','not_applicable','unable'].includes(s)?'Explanation (required)':'Notes (optional)'}</label><textarea id={'note-'+item.id} maxLength={4000} value={a.note} onChange={e=>answer(item.id,'note',e.target.value)}/>
