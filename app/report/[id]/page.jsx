...
export default async function ReportPage({params,searchParams}) {
 ...
 if(!report){
  ...
  w.returnTo=listReturn(query.from,link.data.company_id)
 ...
 </article><EvidencePanel reportId={report.id}/><ReportTools initialReason={actions.data.some(a=>a.id===query.action)?('Follow-up to observation: '+actions.data.find(a=>a.id===query.action).observation).slice(0,4000):''} report={report} actor={user.id} canAmend={editable} returnTo={w.returnTo}/></WorkspaceShell>
}
