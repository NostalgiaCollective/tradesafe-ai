...
export default async function ReportPage({params,searchParams}) {
 ...
 if(report.lifecycle==='draft'){const query=await searchParams;const initialStep=[2,3,4].includes(Number(query.step))?Number(query.step):2;return <WorkspaceShell {...w}><ReportEditor key={report.id} report={report} actor={user.id} editable={editable} initialStep={initialStep}/></WorkspaceShell>}
 ...
 <ul className="review-list">{report.template_snapshot.items.map(i=>{const a=report.document.answers[i.id];return <li key={i.id}><small>{i.category}</small><strong>{i.question}</strong><span>{ANSWERS[answerState(a?.state)]}</span>{a?.note&&<p>{a.note}</p>}{a?.controls&&<p>Immediate controls: {a.controls}</p>}</li>})}</ul>
