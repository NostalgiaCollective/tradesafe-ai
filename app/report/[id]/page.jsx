...
export default async function ReportPage({params,searchParams}) {
 ...
 if(report.lifecycle==='draft'){const query=await searchParams;const initialStep=[2,3,4,5].includes(Number(query.step))?Number(query.step):2;return <WorkspaceShell {...w}><ReportEditor key={report.id} report={report} actor={user.id} editable={editable} initialStep={initialStep} stagingBuild={stagingBuild}/></WorkspaceShell>}
 ...
 </article><EvidencePanel reportId={report.id}/><ReportTools report={report} canAmend={editable}/></WorkspaceShell>
}
