...
export async function POST(request){
 try{
  ...
  try{for(;;){const {done,value}=await reader.read();if(done)break;size+=value.length;if(size>90000){await reader.cancel();throw new AppError('invalid_request')}chunks.push(value)}}finally{reader.releaseLock()}
  let body;try{body=JSON.parse(Buffer.concat(chunks).toString('utf8'))}catch{throw new AppError('invalid_request')}
  if(!['create','save','record','revise','acknowledge','review_control'].includes(body?.command)||!body.payload||typeof body.payload!=='object')throw new AppError('invalid_request')
  const {data,error}=await supabase.rpc('ts_brief_command',{command:body.command,p:body.payload})
