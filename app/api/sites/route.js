...
export async function POST(request){
 try{
  ...
  if(!['create','save','archive','restore','create_brief','create_report','link_brief','link_report'].includes(body?.command)||!body.payload||typeof body.payload!=='object')throw new AppError('invalid_request')
  const {data,error}=await supabase.rpc('ts_site_command',{command:body.command,p:body.payload})
