...
export async function POST(request){
 try{
  ...
  try{for(;;){const {done,value}=await reader.read();if(done)break;size+=value.length;if(size>6000){await reader.cancel();throw new AppError('invalid_request')}chunks.push(value)}}finally{reader.releaseLock()}
  ...
  const {data,error}=await supabase.rpc('ts_review_brief_content',{p:body})
