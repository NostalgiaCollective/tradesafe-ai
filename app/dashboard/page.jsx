...
export default async function DashboardPage({searchParams}){
 ...
 let attention=w.supabase.from('ts_actions').select('id,observation,state,target_date,report:ts_reports(id,document)',{count:'exact'}).eq('company_id',company).neq('state','closed')
 if(worker)attention=attention.eq('responsible_id',w.user.id)
 const [drafts,finalized,actions]=await Promise.all([
 ...
 attention.order('target_date',{nullsFirst:false}).order('id').limit(4)])
 for(const r of [drafts,finalized,actions])if(r.error)throw databaseError(r.error)
 return <WorkspaceShell {...w}>{pending}<div className="daily-intro"><p className="eyebrow">Your working day</p><h1>Ready for the next job.</h1><p>Pick up where you left off, or record a new visit.</p><Link className="primary button" href={'/report/new?company='+company}>Start new report</Link></div><section className="daily-section"><div className="section-heading"><h2>Resume drafts</h2><Link href={library+'&status=draft'}>View all drafts</Link></div><p className="work-footnote">{drafts.count} of your drafts{drafts.count>3?' · showing the 3 most recently saved':''}</p><ReportRows reports={drafts.data} returnTo={returnTo} empty="No drafts to resume. Start a report when you arrive at your next job."/></section><section className="daily-section"><div className="section-heading"><h2>Needs attention</h2><Link href={'/actions?company='+company+(worker?'':'&mine=0')}>{worker?'Assigned to me':'View crew actions'}</Link></div><p>{actions.count} open follow-ups {worker?'assigned to you':'in your company'}{actions.count>4?' · showing the next 4 by due date':''}.</p>{actions.data.length?<ul className="work-list attention-rows">{actions.data.map(a=><li key={a.id}><Link href={'/actions?company='+company+(worker?'':'&mine=0')+'#action-'+a.id}><strong>{a.report?.document.job.address||'Original report'}</strong><span>{a.observation}</span><small>{a.state==='awaiting_verification'?'Awaiting supervisor verification':'Follow-up required'} · {a.target_date?'Due '+a.target_date:'No due date'}</small></Link></li>)}</ul>:<p className="empty-work">{worker?'Nothing assigned to you needs follow-up. Company actions remain available in Actions.':'No open company follow-ups. Finalized findings remain in your report library.'}</p>}</section><section className="daily-section"><div className="section-heading"><h2>Recently finalized</h2><Link href={library+'&status=finalized'}>View finalized reports</Link></div><ReportRows reports={finalized.data} returnTo={returnTo} empty="Finalized reports will appear here. Resume a draft to review and finalize it."/></section><Link className="button library-entry" href={library}>Search / view all reports</Link></WorkspaceShell>
}
