...
export default async function SitePage({params,searchParams}){
 ...
 const [links,counts]=await Promise.all([supabase.from('ts_site_links').select('*,brief:ts_briefs(*),report:ts_reports(id,document,lifecycle,amendment_of,author_id)').eq('company_id',company).eq('site_id',id).order('linked_at',{ascending:false}).limit(50),actionCounts(supabase,company,w.user.id,false,id)])
 if(links.error)throw databaseError(links.error)
 const briefs=links.data.filter(l=>l.brief).map(l=>l.brief),reports=links.data.filter(l=>l.report).map(l=>l.report)
 const back=typeof p.from==='string'?safeRedirect(p.from):'',validBack=back.startsWith('/sites?')&&new URL(back,'https://internal.invalid').searchParams.get('company')===company
