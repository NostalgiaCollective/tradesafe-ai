...
export default function SiteReturn({id,company,actor,from}){
 ...
 useEffect(()=>{
  ...
  const valid=value=>typeof value==='string'&&(safeRedirect(value).startsWith('/sites?')||safeRedirect(value).startsWith('/my-work?'))&&new URL(value,'https://internal.invalid').searchParams.get('company')===company
 ...
 return <Link href={back+(back.startsWith('/sites?')?'#site-'+id:'')}>{back.startsWith('/my-work?')?'Back to My work':'Back to sites'}</Link>
}
