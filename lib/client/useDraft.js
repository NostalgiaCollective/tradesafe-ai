...
export function useDraft(initial,actor) {
 ...
 const save=useCallback(async()=>{
  ...
  running.current=(async()=>{
   try{
    const r=await command('save_report',payload);revision.current=r.revision;confirmed.current=documentKey(r.document);pending.current=null
