...
export function useDraft(initial,actor,saveCommand=command) {
 ...
 const change=useCallback(update=>{const next=typeof update==='function'?update(current.current):update;current.current=next;setDocument(next);setStatus('Not saved')},[])
 const save=useCallback(async()=>{
  ...
  running.current=(async()=>{
   try{
    const r=await saveCommand('save_report',payload,actor);revision.current=r.revision;confirmed.current=documentKey(r.document);pending.current=null
 ...
 },[initial.id,initial.company_id,actor,saveCommand])
 useEffect(()=>{
