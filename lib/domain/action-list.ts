...
export function actionFilters(params:Record<string,unknown>){
 return {mine:params.mine!=='0',status:Object.hasOwn(ACTION_VIEWS,String(params.status))?String(params.status):params.closed==='1'?'all':'outstanding',page:Math.max(0,Math.min(10000,Number.parseInt(String(params.page||'0'),10)||0)),focus:typeof params.focus==='string'&&UUID.test(params.focus)?params.focus:''}
}
export function actionListUrl(company:string,filters:ReturnType<typeof actionFilters>){return '/actions?'+new URLSearchParams({company,mine:filters.mine?'1':'0',...(filters.status!=='outstanding'?{status:filters.status}:{}),...(filters.page?{page:String(filters.page)}:{}),...(filters.focus?{focus:filters.focus}:{})})}
