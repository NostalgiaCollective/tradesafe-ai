...
import { MAX_IMAGE_BYTES,MAX_NORMALIZED_IMAGE_BYTES } from './limits.mjs'
export { MAX_IMAGE_BYTES,MAX_NORMALIZED_IMAGE_BYTES } from './limits.mjs'
export const digest = bytes => createHash('sha256').update(bytes).digest('hex')

export async function readImageBody(request,timeoutMs=30000) {
 ...
 const deadline=new Promise((_,reject)=>{timer=setTimeout(()=>{reject(new AppError('request_timeout'));void reader.cancel().catch(()=>{})},timeoutMs)})
 try { while(true){const {value,done}=await Promise.race([reader.read(),deadline]);if(done)break;size+=value.byteLength
...
export async function validateImage(input) {
 ...
 try {
  ...
  if(data.length>MAX_NORMALIZED_IMAGE_BYTES)throw new Error('Output too large')
