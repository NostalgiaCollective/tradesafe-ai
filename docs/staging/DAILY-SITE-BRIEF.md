...
One new migration: `20260921000100_daily_briefs.sql`. It adds brief/draft, immutable version, acknowledgement and review tables with read RLS and command-only writes. The existing Actions table gains a second, company-checked origin; exactly one report or brief origin is required. Making report_id nullable under that exclusive-origin constraint removes no data and weakens no company/role permission. Applied migrations are untouched.
