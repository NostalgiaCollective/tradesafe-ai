...
Account recovery follow-up: a rejected changed account does not permanently block the original account's explicit retry. A focused real-browser check retained the entered action text, rejected the substituted owner session, restored the original worker session without credential changes, saved on explicit retry and verified the value after reload (`.staging/interruption-original-account-retry.json`). This case is included in the hosted harness.
