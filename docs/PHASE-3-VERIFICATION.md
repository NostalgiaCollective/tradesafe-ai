...
See [implementation and design API contract](PHASE-3-EVIDENCE-EXPORTS.md) for permissions, endpoint/error shapes, current photo limits (5 MiB input, 3 MiB normalized storage, 20 MP, 10 photos), non-atomic upload reconciliation, tombstone cleanup, two-minute export lease, immutable versioned exports and required server/font/native-runtime setup. The original verification above predates the [5 MiB input increase](staging/PHOTO-5-MIB.md). No signed URLs are issued: every new byte request rechecks membership. Already-delivered bytes cannot be recalled. Later corrective-action history is explicitly excluded from the original PDF and remains in the authorized Actions view; amendment indexing is labelled with the export cutoff.
